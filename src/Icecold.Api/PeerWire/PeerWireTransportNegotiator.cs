    ...
    ILogger<PeerWireTransportNegotiator> logger)
{
    ...
    public async Task<Stream?> NegotiateAsync(Stream stream, CancellationToken cancellationToken)
    {
        ...
        if (firstByte == PeerWireProtocol.ProtocolName.Length)
        {
            ...
            return await NegotiateMseAsync(stream, prefix, cancellationToken);
        }

        return await NegotiateMseAsync(stream, [(byte)firstByte], cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!await PeerWireStreamIo.ReadExactlyOrFalseAsync(stream, peerPublicKey.AsMemory(publicKeyPrefix.Length), cancellationToken))
                return null;
            ...
            if (!await ReadUntilNeedleAsync(stream, PeerWireMse.HashReq1(secret), PeerWireMse.MaxPadLength, cancellationToken))
                return null;

            var xor = new byte[20];
            if (!await PeerWireStreamIo.ReadExactlyOrFalseAsync(stream, xor, cancellationToken))
                return null;
            ...
            var header = await ReadEncryptedAsync(stream, decrypt, 14, cancellationToken);
            ...
            if (padCLength > 0 && await ReadEncryptedAsync(stream, decrypt, padCLength, cancellationToken) is null)
                return null;

            var initialPayloadLengthBytes = await ReadEncryptedAsync(stream, decrypt, 2, cancellationToken);
            ...
            var initialPayload = initialPayloadLength == 0
                ? Array.Empty<byte>()
                : await ReadEncryptedAsync(stream, decrypt, initialPayloadLength, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await PeerWireStreamIo.ReadExactlyOrFalseAsync(stream, buffer, cancellationToken))
            return null;
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var value = await PeerWireStreamIo.ReadByteAsync(stream, cancellationToken);
            if (value < 0)
                return false;
